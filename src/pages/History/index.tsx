...
function HistoryPage() {
    ...
    const calculateRange = (num: number) => {
        const range = [];
        let i = 1;
        for (let i = 1; i <= num; i++) {
            range.push(i);
    ...
    async function getUserPunches(page: number) {
        try {
            ...
            if (response.status === 200) {
                ...
                calculateRange(response.data.maxPages);
            }
        } catch (e) {
        }
    }
    ...
    return (
        <Layout>
            ...
            <DivTable>
                ...
                <TableFooter>
                    {range?.map((val, key) => (
                        <Button key={key} onClick={() => handleChangePage(val)} disabled={page === val}>{val}</Button>
                    ))}
                </TableFooter>
            </DivTable>
        </Layout>
